...
function CompararMix ({ cancion, title }) {
  ...
  return (
    ...
    <div className='flex justify-center items-center gap-10 mt-10 w-full md:w-[70%] rounded-full m-auto h-12 md:h-24 shadow-xl bg-red-50'>
      ...
      <div className='flex justify-center items-center cursor-pointer rounded-full w-12 h-12 md:w-24 md:h-24 bg-red-500 hover:scale-110 duration-300' onClick={handlePlayPause}>
      ...
      <h3 className='w-full text-center text-sm md:text-3xl text-red-500 font-semibold'>{title}</h3>
      <div className={`${mixxed ? 'bg-red-500 text-white' : 'text-red-500'} flex justify-center items-center cursor-pointer rounded-full w-12 h-12 md:w-24 md:h-24 border-2 border-red-500 hover:border-none hover:scale-110 hover:bg-red-500 duration-300`} onClick={() => setMixxed(!mixxed)}><Button className='hover:text-white duration-300 text-sm md:text-xl'>Mix!</Button></div>
    </div>
  )
}
